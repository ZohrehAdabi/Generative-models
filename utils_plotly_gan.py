...
def read_hdf_all_epochs_train(file):
    ...
    return df_sample, df_data, df_intermediate_smpl_list, df_score_info.reset_index(drop=True), df_score_info_hist, df_grad_g_z_list, epoch_list, timesteps_list, n_samples, grid_size
...
def prepare_plotly_fig_quiver_grad_old(df_sample, df_data, fake_sample, df_grads, epoch_list, 
    ...
    for i in epoch_list:
        ...
        if i==epoch_list[0]:
            fig_quiver = ff.create_quiver(x, y, u, v, scale=scale, name=quiver_name, arrow_scale=.2,
                        angle=15*np.pi/180, line=dict(width=1, color=color))
...
def prepare_plotly_fig_quiver_grad(df_sample, df_data, df_intermediate_smpl_list, df_grid_samples,
                                ...
                                epoch_list, timesteps_list, grid_size, quiver_name, animation_frame, method,
                                width=750, height=570, range_x=[-2.5, 2.5], range_y=[-2.5, 2.5]):


    # df_sample = df_sample.reset_index(drop=True)
    animation_names = [f'Epoch {e}' for e in epoch_list]
    # animation_names.insert(0, "All zeros")
    # epoch_list.insert(0, 0)
    colors = dict(sample='#AB63FA', grad='DarkSlateGrey', data='#19D3F3') # samples grads data
    # create frames for each animation [epochs]

    # df_sample_list = [df_sample] + df_intermediate_smpl_list # All zeros + other validation epoch
    animation_list, slider_list = [], []
    df_sample = df_sample[::-1] # last sample will be at the end of animation
    ...
    # animation_list.append(all_zero_sample_frames)
    # slider_list.append(all_zero_slider)
    
    for i, (e, dfs) in enumerate(zip(epoch_list, df_intermediate_smpl_list)): #traces: data samples grads

        dfs = dfs[::-1] # last sample will be at the end of animation
        ...
        sample_frames = [go.Frame(data=[go.Scatter(x=frame['data'][0].x, y=frame['data'][0].y,  mode='markers', showlegend=True,
                                     ...
                                     ], traces=[3+i], name=f'frame_{e}_{t}', group=f'epoch_{e}')  for t, frame in zip(timesteps_list[::-1], px_anime_sample.frames)]

        animation_list.append(sample_frames)

    n_button_animations = len(animation_list) + 1  # 1: for all_zeros button # equals to plays buttons
    ...
    trace_names = ['data', 'sample_10', 'grad_10', 
                   #'sample_10', 
                   'sample_20', 'sample_30', 'sample_40', 'sample_50', 'sample_60', 'sample_70', 'sample_80', 'sample_90', 'sample_100', 
                   'Loss D', 'Loss G', 'Loss D mean', 'Loss G mean', 
                   'Loss D', 'Loss G', 'Loss D mean', 'Loss G mean', 
                   'D(x)', 'D(G(z))', 
                   'D(x) std', 'D(G(z)) std', 'D(x) mean', 'D(G(z)) mean']
    
    trace_visibility['data'] = True
    trace_vis_init = trace_visibility.copy()
    # trace_vis_init['data'] = True
   ...
   # add play buttons for all epochs
    for i, e in enumerate(epoch_list):
        ...
        slider_steps.update(slider_info.copy())
    ...
    show_stats = {'title': f'Loss-Score', 'width':1000, 'height': 450, 'xaxis.autorange': True, 'yaxis.autorange': True} 
    ...
    for i in range(n_button_statistics):  
        ...
        loss_button = dict(buttons=[dict(label=f'{stats_lables[i]}', method='update', args=[{'visible': list(stats_vis.values())}, 
                                                                                            {'title': f'{stats_lables[i]}'}])])
        loss_button.update(button_info)
        loss_button.update(dict(visible=False, x=0.15+((i+1))/8, y=1.02, pad={"r": 10, "t": 25, "l": 10}))
        loss_button_list.append(loss_button)
    ...
    n_epoch = 1000
    lr = 0.001
    text = rf"$\text{{GAN loss comb, cuda, ...  n_epoch}}  = {n_epoch}, \ell r = {lr:.0e}, \delta_3 = 10 \\ \text{{lr_dsc 1e-4, lr_gen 1e-4, z_dim gen dsc model}} \\ \text{{ epoch seed 2 n_sample time_steps}}$"
    show_detail['title.text'] = text.replace("e-0", "e-")
    show_detail['sliders'] = []
    ...
    show_detail.update(dict(plot_bgcolor='white', height=350))
    ...
    hide_info = {'margin.t': 60, 'title.y': 0.98, 'title.x':0.001, 'showlegend': True, 'plot_bgcolor': None, 'width':750, 'height': 570,
                'xaxis.tickvals': None, 'yaxis.tickvals': None, 'xaxis.range': [-2.5, 2.5], 'yaxis.range': [-2.5, 2.5], 'xaxis.title': 'x', 'yaxis.title': 'y'}
    ...
    button_list = play_button_list + loss_button_list + [pause_button] + [back_stats_button] + [back_detail_button]
    ...
    fig.update_layout(
        title=dict(text='Animation Example', pad={"r": 10, "t": 10, "l": 20, "b":10}, x=0.001, xanchor="left", y=0.98, yanchor="top"),
        xaxis=dict(title=dict(text="x", font_size=12), range=[-2.5, 2.5],  showgrid=True, autorange=False),
        yaxis=dict(title=dict(text="y", font_size=12), range=[-2.5, 2.5],  showgrid=True, autorange=False),
...
def plot_animation(method, expr_id, training=True, epoch_key=-1, all_epochs=False, quiver_name=None, test_name=None):
    ...
    #         # df_fake_sample = pd.DataFrame(fake_samples, columns=['x', 'y'])
            # df_sample, animation_frame = \
            #     prepare_df_all_epochs_for_plotly(df_sample, df_data, df_grad_g_z, df_score_info, epoch_list, timesteps_list, n_samples, grid_size)       
    ...
    if training and all_epochs:
        ...
        if quiver_name is None:
            ...
            fig_all = prepare_plotly_fig_quiver_grad(df_sample, df_data, df_intermediate_smpl_list, df_grid_samples, 
                                                                df_grad_g_z_list, df_score_info, df_score_info_hist,
                                                                df_loss_dsc_itr, df_loss_gen_itr, df_loss, df_score,
                                                                epoch_list, timesteps_list, grid_size, quiver_name, animation_frame, method_type)
