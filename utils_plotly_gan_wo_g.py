...
def read_hdf_loss(file):
    ...
    df_loss_gen_itr = pd.DataFrame(df_loss.loc[0, ['loss_fk_dt_itr']].values[0], columns=['loss_fk_dt_itr'])
    ...
    for i in range(1, df_loss.shape[0]):
        ...
        df_loss_gen_itr = pd.concat([df_loss_gen_itr, pd.DataFrame(df_loss.loc[i, ['loss_fk_dt_itr']].values[0], columns=['loss_fk_dt_itr'])])
    ...
    return df_loss.loc[:, ['epoch', 'loss_dsc_epoch', 'loss_fk_dt_epoch']], df_loss_dsc_itr, df_loss_gen_itr, df_score
...
def add_statistics_to_fig(fig, df_loss_dsc_itr, df_loss_gen_itr, df_loss, df_score):
    ...
    fig.add_trace(go.Scatter(x=x, y=df_loss_gen_itr['loss_fk_dt_itr'], name="Loss F", mode='lines', visible=False, showlegend=True, line=dict(width=1, color='crimson'))) 
    ...
    fig.add_trace(go.Scatter(x=x, y=(np.cumsum(df_loss_gen_itr['loss_fk_dt_itr']) / x), name="Loss F mean", mode='lines', visible=False, showlegend=True, line=dict(width=2, color='deeppink')))
    x = df_loss['epoch']
...
def visibility_buttons_and_idx(n_all_buttons, n_button_animations, n_button_statistics):
    ...
    return button_visibility, fake_idx, stats_idx, pause_idx, back_stats_idx, back_detail_idx, select_dropdown_idx
...
def prepare_plotly_fig_quiver_grad(df_sample, df_data, df_intermediate_smpl_list, df_fake_data,
                                   ...
                                   df_score_info_hist, df_loss_dsc_itr, df_loss_gen_itr, 
    ...
    # store idx of other buttons that adding to the layout for making visible True or False
    # buttons visibility
    button_visibility, fake_idx, stats_idx, pause_idx, \
    ...
    button_list = play_button_list + [fake_data_button] + stats_button_list + [pause_button] + [back_stats_button] + [back_detail_button]
